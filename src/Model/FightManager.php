...
class FightManager extends AbstractManager
{
    ...
    {
        $statement = $this->pdo->prepare('SELECT DATE_FORMAT(fight.date, "%Y-%m-%d at %h:%i") AS date, monster.name as monster_name, fight.hunter_points, fight.monster_points 
            FROM ' . $this->table . ' JOIN monster 
            ON monster.id=fight.monster_id WHERE hunter_id=:id ORDER BY date DESC LIMIT 5');
